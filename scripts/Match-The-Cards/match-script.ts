...
const gameContainer = document.getElementById("main-grid") as HTMLDivElement;
...
function renderCards() {
    ...
    cards.forEach(card => {
        ...
        cardEl.className = `w-20 h-28 rounded-xl flex items-center justify-center text-4xl font-bold transition duration-300 shadow-xs shadow-yellow-400 ${
        card.isFlipped || card.isMatched ? "bg-red-700 text-white cursor-default" : "bg-neutral-700 cursor-pointer hover:scale-105 hover:-translate-y-1"
...
function handleCardClick(id: number) {
    ...
    if (!card || card.isFlipped || card.isMatched || flippedCards.length === 2) return; //to handle if the card is already flipped or matched or if there are already 2 flipped cards
